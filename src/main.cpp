...
int main(int argc, char** argv)
{
	...
	parser.add<io::SpawnSwordsman>([&](auto cmd)
	{
		if (!map.isInBounds(cmd.x, cmd.y))
			throw std::runtime_error("SPAWN_SWORDSMAN: coordinates (" + std::to_string(cmd.x)
			    + "," + std::to_string(cmd.y) + ") are out of map bounds");
		if (map.isOccupied(cmd.x, cmd.y))
			throw std::runtime_error("SPAWN_SWORDSMAN: cell (" + std::to_string(cmd.x)
			    + "," + std::to_string(cmd.y) + ") is already occupied");
	...
	parser.add<io::SpawnHunter>([&](auto cmd)
	{
		if (!map.isInBounds(cmd.x, cmd.y))
			throw std::runtime_error("SPAWN_HUNTER: coordinates (" + std::to_string(cmd.x)
			    + "," + std::to_string(cmd.y) + ") are out of map bounds");
		if (map.isOccupied(cmd.x, cmd.y))
			throw std::runtime_error("SPAWN_HUNTER: cell (" + std::to_string(cmd.x)
			    + "," + std::to_string(cmd.y) + ") is already occupied");
